...
n_estimators_to_test = [1, 2, 5, 10, 20, 50, 75, 100]
...
for n_est in n_estimators_to_test:
    ...
    base_model_fixed = RandomForestRegressor(
        n_estimators=n_est,
...
print(f"\nEfficiency Analysis:")
print(f"Most efficient n_estimators (70% MSE performance + 30% simplicity): {best_efficiency_n}")
...
if SHOWPLOTS:
    ...
    plt.plot(nest_results['n_estimators'], efficiency_scores, 'o-', label='Efficiency Score (0.7×MSE + 0.3×Simplicity)', color='green', alpha=0.7, linewidth=2)
    ...
    plt.title("Random Forest: Histogram of True vs Predicted Values")
    ...
    plt.title("Random Forest: Empirical CDF of True vs Predicted Values")
    plt.xlabel("Number of Passengers")
    ...
    plt.title("Random Forest: Top 15 Permutation Feature Importances")
    plt.tight_layout()
    ...
    # Plot out-of-bag score progression (if available)
    if hasattr(best_model, 'oob_score_') and best_model.oob_score is True:
...
if PRICESENS:
    ...
    if 'price' in X_val_scaled.columns:
        ...
        plt.ylim(0, 1000)
        plt.title('Random Forest: Partial Dependence of Passengers on Price\n(baselines with train_type=1)')
