...
# --- Config ---
SAVED_MODEL_PATH = os.path.join(project_root, "ConstraintLearning/saved_models/demand_rf_model.pkl")
    ...
    for delta in DELTA_VALUES:
        ...
        if opt_m.status in [gp.GRB.OPTIMAL, gp.GRB.INTERRUPTED, gp.GRB.TIME_LIMIT]:
            try:
                ...
                filename = f"results_rf_{day}_delta_{delta}_obj_{objective_str}.csv"
                
                # Create results_rf directory if it doesn't exist
                results_dir = os.path.join(os.path.dirname(__file__), 'results_rf')
