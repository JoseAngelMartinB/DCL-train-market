...
for day in DAYS:
    for delta in DELTA_VALUES:
        ...
        for i in range(n_trains_context):
            ...
            if train_type_AVE == 1 or train_type_AVLO == 1:
                pL, pU = price_bounds[i]
                dL, dU = 0.0, float(capacity_values[i])
                revenue_i = opt_m.addVar(
                    lb=0.0,
                    ub=pU * dU,
                    name=f"revenue_{i}"
                )
                # McCormick envelope for revenue_i = price_vars[i] * ActualDemands[i]
                opt_m.addConstr(
                    revenue_i >= pL * ActualDemands[i] + dL * price_vars[i] - pL * dL,
                    name=f"mccormick_lb1_revenue_{i}"
                )
                opt_m.addConstr(
                    revenue_i >= pU * ActualDemands[i] + dU * price_vars[i] - pU * dU,
                    name=f"mccormick_lb2_revenue_{i}"
                )
                opt_m.addConstr(
                    revenue_i <= pU * ActualDemands[i] + dL * price_vars[i] - pU * dL,
                    name=f"mccormick_ub1_revenue_{i}"
                )
                opt_m.addConstr(
                    revenue_i <= pL * ActualDemands[i] + dU * price_vars[i] - pL * dU,
                    name=f"mccormick_ub2_revenue_{i}"
                )

                revenue_vars.append(revenue_i)
        ...
        if opt_m.status in [gp.GRB.OPTIMAL, gp.GRB.INTERRUPTED, gp.GRB.TIME_LIMIT]:
            try:
                ...
                # Obtain total RENFE revenue from results_df for AVE and AVLO trains
                real_total_revenue = results_df[results_df['train_type'].isin(['AVE', 'AVLO'])].apply(lambda row: row['optimized_price'] * row['actual_demand'], axis=1).sum()
                ...
                new_row = {
                    ...
                    'day': day,
                    'delta': delta,
                    'total_revenue': real_total_revenue,
                    'gap': opt_m.MIPGap,
