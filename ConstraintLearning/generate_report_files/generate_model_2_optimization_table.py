...
def model_tuple_order(model_tuple):
    try:
        demand_model, price_model = [x.strip() for x in model_tuple.split("+")]
    except ValueError:
        return (len(MODEL_ORDER), len(MODEL_ORDER))

    demand_order = MODEL_ORDER.index(demand_model) if demand_model in MODEL_ORDER else len(MODEL_ORDER)
    price_order = MODEL_ORDER.index(price_model) if price_model in MODEL_ORDER else len(MODEL_ORDER)
    return (demand_order, price_order)
...
def build_latex_table(df):
    ...
    lines.append(r"            \begin{tabular}{@{}c@{}}\textbf{Model tuple}\\\textbf{\scriptsize (Demand + Price)}\end{tabular}")
    ...
    for model_tuple, tuple_df in df.groupby("model_tuple", sort=False):
        tuple_nrows = len(tuple_df)
        first_tuple_row = True

        for delta, delta_df in tuple_df.groupby("delta", sort=False):
            delta_nrows = len(delta_df)
            first_delta_row = True

            for _, row in delta_df.iterrows():
                tuple_cell = f"\\multirow{{{tuple_nrows}}}{{*}}[-1.5ex]{{{model_tuple}}}" if first_tuple_row else ""
...
def main():
    ...
    df["model_tuple"] = df["ml_model"].apply(parse_model_tuple)

    grouped = (
        df.groupby(["model_tuple", "delta", "day"], as_index=False)
        .agg(
    ...
    grouped["tuple_order"] = grouped["model_tuple"].apply(model_tuple_order)
    grouped["day_order"] = grouped["day"].apply(
        lambda x: DAY_ORDER.index(str(x)) if str(x) in DAY_ORDER else len(DAY_ORDER)
    )

    grouped = grouped.sort_values(["tuple_order", "delta", "day_order"]).drop(
        columns=["tuple_order", "day_order"]
    )
