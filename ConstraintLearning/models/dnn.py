...
def bounded_relu(x, max_val):
    return F.relu(x) - F.relu(x - max_val)
...
class FeedForwardNN(nn.Module):
    def __init__(self, input_size, output_size, hidden_layers, drop):
        ...
        # First Layer
        first_lin_layer = nn.Linear(self.input_size, self.hidden_layers[0])
        ...
        #    [nn.BatchNorm1d(self.hidden_layers[i]) for i in range(len(self.hidden_layers))]
        #)
    ...
    def forward(self, x):
        ...
        #return F.relu(x) # Force positive outputs, i.e. x >= 0

        #out = self.output_layer(x)
        #return bounded_relu(out, max_val=1000)
